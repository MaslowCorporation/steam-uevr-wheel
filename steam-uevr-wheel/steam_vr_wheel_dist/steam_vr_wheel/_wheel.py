...
class Wheel(RightTrackpadAxisDisablerMixin, VirtualPad):
    def __init__(self, inertia=0.95, center_speed=pi/180):
        ...
        # TODO: Init the float sending mechanism here
        # for socket communication between python and the outside world (C++ plugin ;-)
        self.sender = FloatSenderZMQ()
        self.sender.connect()
    ...
    def render(self):
        ...
        # TODO: Add wheel angle socket transmission here ;-)
        wheel_angle_deg = wheel_angle * 180.0 / PI
        self.sender.send_float(wheel_angle_deg)
...
# TODO: Implement the socket client feeding the C++ socket server (inside the dllmain.cpp UEVR plugin)
class FloatSenderSocket:
    ...
    def connect(self):
        try:
            ...
            print("Connected to the socket server at port %s ;-)" % (self.port))
        except Exception:
            print("We couldn't connect to the socket server at port %s" % (self.port))
    ...
    def send_float(self, float_value):
        if self.is_connected:   
            ...
            self._send_float(float_value)
        else:
            self.connect()


    def _send_float(self, float_value):
        try:
            float_bytes = struct.pack('f', float_value)

                # Send the float bytes
            self.socket.sendall(float_bytes)
        except Exception:
            print("Oops... we couldn't send the float %s" % (float_value))
    ...
    def close(self):
        if self.socket:
            self.socket.close()
            print("Socket client connection stopped ;-)")




# The ZMQ client
class FloatSenderZMQ:
    def __init__(self, host='localhost', port=75832):
        self.host = host
        self.port = port
    ...
    def _send_float(self, float_value):
        try:
            ...
            #self.socket.recv()  # Wait for the server's response to maintain REQ/REP pattern
        except Exception as e:
            print("Oops... we couldn't send the float")
            self.is_connected = False
