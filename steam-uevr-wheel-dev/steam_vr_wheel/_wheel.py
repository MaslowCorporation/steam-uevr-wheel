...
class SteeringWheelImage:
    ...
    def move(self, point, size):
        ...
        #print("Wheel coords: %s, %s, %s" % (point.x, point.x, point.z))
        self.size = size
        fn = self.vroverlay.function_table.setOverlayTransformAbsolute
...
class Wheel(RightTrackpadAxisDisablerMixin, VirtualPad):
    ...
    def wheel_raw_angle(self, point):
        ...
        #print("Wheel angle: %s , center x:  %s , center y:  %s , center z:  %s , ctrl x: %s , ctrl y: %s , ctrl z: %s" % (angle, self.center.x, self.center.y, self.center.z, point.x, point.y, point.z))
    ...
    def wheel_double_raw_angle(self, left_ctr, right_ctr):
        ...
        #print("Wheel angle (double): %s" % (angle))
    ...
    def calculate_grab_offset(self, raw_angle=None):
        ...
        #print("Grab offset calculation")
        self.config.wheel_grab_offset = self._wheel_grab_offset
    ...
    def render(self):
        ...
        # TODO: Add wheel angle travel here ;-)
        self.sender.send_float(wheel_angle_deg)

        #print("Wheel angle: %s" % (wheel_angle_deg))
...
class FloatSenderSocket:
    ...
    def connect(self):
        ...
        print("Connecting to the VR plugin server on port " +  str(self.port))
        self.socket = context.socket(zmq.PUSH)
        self.socket.connect("tcp://localhost:" + str(self.port))

    def send_float(self, float_value):
        self.socket.send_string(str(float_value))
